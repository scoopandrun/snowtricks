...
class TrickFixture extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        // Difficulties
        foreach ($this->generateDifficulties() as $difficulty) {
            $this->addReference($difficulty->getName(), $difficulty);
     ...
     */
    private function generateDifficulties(): \Generator
    {
        $difficultiesData = [
            ["value" => 1, "name" => "Easy"],
        ...
        foreach ($difficultiesData as $difficultyData) {
            $difficulty = (new Difficulty())
                ->setValue($difficultyData["value"])
                ->setName($difficultyData["name"]);
    ...
    private function generateTricks(): \Generator
    {
        ...
        foreach ($tricksData as $trickData) {
            $trick = (new Trick)
                ...
                ->setDifficulty($this->getReference($trickData["difficulty"]))
